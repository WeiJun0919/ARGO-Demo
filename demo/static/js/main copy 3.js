...
async function fetchAndUpdateCleaningStatus() {
    try {
        const response = await fetch('/api/get-cleaning-status');
        const result = await response.json();
        ...
        if (result.success) {
            ...
            const decisionTable = result.decision_table;
            ...
            // 3. 更新右侧时间线
            updateTimelineWithIteration(status, decisionTable);

            // 4. 如果训练完成（iteration >= total_iterations），停止轮询
            ...
            console.warn('获取状态成功但数据异常:', result.message);
        }
    } catch (error) {
...
function updateTimelineWithIteration(status, decisionTable) {
    if (!status) return;
    ...
    // 1) 时间线标题 + 当前迭代全局信息（来自 joint_latest.json 顶层）
    let timelineHTML = `
        <div class="timeline-title">
            <i class="fa fa-history"></i>强化学习清洗进度
            <small class="text-gray-dark">(第 ${safeNumber(status.iteration)}/${safeNumber(status.total_iterations)} 轮 | ${status.timestamp || '--'})</small>
        </div>

        <div class="timeline-item">
            <div class="timeline-dot" style="background:#165DFF;"></div>
            <div class="timeline-line"></div>
            <div class="timeline-iteration-title">当前轮次概览（Iteration ${safeNumber(status.iteration)}）</div>
    ...
    // 2) 显示 steps 中每一步：step/action/reward/accuracy/dirty_ratio
    if (decisionTable && decisionTable.length > 0) {
        ...
        sortedSteps.forEach((stepData, index) => {
            ...
            timelineHTML += `
                <div class="timeline-item">
                    ...
                    <div class="timeline-iteration-title">
                        Step ${safeNumber(stepData.step)} ${isLatestStep ? '<span class="badge">最新</span>' : ''}
                    </div>
        ...
        timelineHTML += `
            <div class="timeline-item">
                ...
                <div class="timeline-content-row text-gray-dark">
                    <i>训练启动后将从 joint_latest.json 的 steps 字段实时刷新。</i>
                </div>
            </div>
        `;
    }
    ...
    if (safeNumber(status.iteration) >= safeNumber(status.total_iterations) && safeNumber(status.total_iterations) > 0) {
        timelineHTML += `
            <div class="timeline-item">
                ...
                <div class="timeline-content-row text-success">
                    <i class="fa fa-check-circle"></i> 数据清洗训练已全部完成。
                </div>
                <div class="timeline-content-row"><strong>最终 accuracy:</strong> ${(safeNumber(status.accuracy) * 100).toFixed(2)}%</div>
                <div class="timeline-content-row"><strong>best_accuracy:</strong> ${(safeNumber(status.best_accuracy) * 100).toFixed(2)}%</div>
                <div class="timeline-content-row"><strong>dirty_ratio:</strong> ${(safeNumber(status.dirty_ratio) * 100).toFixed(2)}%</div>
            </div>
        `;
...
// 运行质量检测并加载结果
// 质量检测功能
async function runQualityCheckAndLoad() {
