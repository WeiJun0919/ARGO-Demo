        ...
        try:
            ...
            if target_dataset == 'test':
                ...
                if not current_table_dataset_basename:
                    return jsonify({'error': '请先上传 Test Set 文件，再上传 Validation Set 文件。'}), 400
                preferred_base_name = current_table_dataset_basename
            ...
            return jsonify({'error': f'表格文件转存失败: {str(e)}'}), 400
...
def run_data_cleaning():
    ...
    global training_process, training_log_file
    try:
        # 1. 确定脚本路径
        ...
        # 4. 使用子进程异步启动脚本
        # interactive 模式需要 stdin 管道，以便前端发送 y / a
        # stdout/stderr 不重定向到文件，直接输出到当前 Flask 终端
        ...
        # 保存全局句柄，供 /api/submit-decision 使用
    ...
    try:
        ...
        if not os.path.exists(pending_file):
            return jsonify({
                ...
                'pending': False,
                'data': None,
                'message': 'pending_decision.json 不存在。'
            })
        ...
        return jsonify({
            'success': True,
            'pending': bool(pending_data.get('pending', False)),
            'data': pending_data
        })
    except json.JSONDecodeError as e:
        return jsonify({'success': False, 'error': f'pending_decision.json 解析失败: {str(e)}'}), 500
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def submit_decision_signal():
    ...
    try:
        ...
        if signal not in ['y', 'a']:
            return jsonify({'success': False, 'error': '仅支持信号 y 或 a'}), 400
...
def get_cleaning_status():
    ...
    try:
        ...
        # 追加历史轮次列表（training_log.json），用于时间线展示完整轮次
        history_iterations = []
        history_path = os.path.join(os.path.dirname(json_file_path), 'training_log.json')
        if os.path.exists(history_path):
            try:
                with open(history_path, 'r', encoding='utf-8') as fh:
                    history_data = json.load(fh)
                if isinstance(history_data, list):
                    for iter_item in history_data:
                        history_iterations.append({
                            'iteration': iter_item.get('iteration', 0),
                            'timestamp': iter_item.get('timestamp', ''),
                            'accuracy': iter_item.get('accuracy', 0.0),
                            'best_accuracy': iter_item.get('best_accuracy', 0.0),
                            'dirty_ratio': iter_item.get('dirty_ratio', 0.0),
                            'steps': iter_item.get('steps', [])
                        })
        ...
        return jsonify({
            ...
            'iteration_history': history_iterations,
            'message': '状态获取成功。'
        })
